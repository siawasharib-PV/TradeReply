...
class DatabaseHelper:
    ...
    def init_schema(self):
        ...
        if "google_refresh_token" not in business_cols:
            self.cursor.execute("ALTER TABLE businesses ADD COLUMN google_refresh_token TEXT")

        self.cursor.execute("PRAGMA table_info(reviews)")
        review_cols = {row[1] for row in self.cursor.fetchall()}
        if "google_review_id" not in review_cols:
            self.cursor.execute("ALTER TABLE reviews ADD COLUMN google_review_id TEXT")
        if "google_review_name" not in review_cols:
            self.cursor.execute("ALTER TABLE reviews ADD COLUMN google_review_name TEXT")
    ...
    def get_business(self, business_id: str) -> Optional[Business]:
        ...
        if row:
            return Business(
                ...
                google_refresh_token=row["google_refresh_token"] if "google_refresh_token" in row.keys() else None,
    ...
    def update_business_mapping(
        ...
        try:
            self.cursor.execute(
                ...
                WHERE id = ?
                """,
                (google_location_id, google_account_id, google_refresh_token, response_tone, business_id),
            )
            self.conn.commit()
            return True
        except sqlite3.Error:
